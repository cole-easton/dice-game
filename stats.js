...
function generateGraph(includeOutliers) {

    const graphData = includeOutliers?sortedData:filteredData;

    const chartMin = graphData[graphData.length - 1];
    const chartMax = graphData[0];
    ...
    for (let i = 0; i < graphData.length; i++) {
        const bin = Math.floor((graphData[i] - chartMin) / step);
        if (0 <= bin && bin < bars) {
            bins[bin]++;
        }
        else if (graphData[i] === chartMax) {
            bins[bars - 1]++;
        }
    }
    ...
    for (let i = 0; i < bars; i++) {
        ...
        const upper = i===bars-1?chartMax:Math.ceil(chartMin + (i + 1) * (step) - 1);
...
const checkbox = document.getElementById("outliers");
checkbox.onchange = e => {
    console.log(e.target.checked);
    generateGraph(e.target.checked);
}

generateGraph(checkbox.checked);
